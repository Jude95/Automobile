...
import com.jude.automobile.ui.BrandAddActivity;
 ...
 */
public class BrandAddPresenter extends BeamDataActivityPresenter<BrandAddActivity,Brand> {
    ...
    @Override
    protected void onCreate(BrandAddActivity view, Bundle savedState) {
        super.onCreate(view, savedState);
        provider = new ImageProvider(getView());
        data = (Brand) getView().getIntent().getSerializableExtra("data");
        if (data == null) data = new Brand();
        publishObject(data);
    }
    ...
    public void publishEdit(){
        if(TextUtils.isEmpty(data.getAvatar())){
            JUtils.Toast("请选择品牌标志");
            return;
        }
        if(TextUtils.isEmpty(data.getName())){
            JUtils.Toast("请填写品牌名字");
            return;
        }
        if(TextUtils.isEmpty(data.getWord())){
            JUtils.Toast("请填写品牌搜索关键字");
            return;
        }
    ...
    public void delete(){
        ...
        DataModel.getInstance().deleteBrand(data.getId())
                .compose(new ErrorTransform<>(ErrorTransform.ServerErrorHandler.AUTH_TOAST))
